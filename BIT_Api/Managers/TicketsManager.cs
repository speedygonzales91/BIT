...
namespace BIT_Api.Managers
{
    public class TicketsManager
    {
        ...
        public TicketsManager(ApplicationDbContext db, IMapper mapper)
        {
            _db = db;
            _mapper = mapper;
        }
        ...
        public async Task<TicketDTO> CreateTicket(TicketDTO ticketDTO)
        {
            try
            {
                ...
                foreach (var commentDTO in ticketDTO.Comments)
                {
        ...
        public async Task<TicketDTO> GetTicket(int ticketId)
        {
            try
            {
                var ticket = _mapper.Map<Ticket, TicketDTO>(await _db.Tickets.Include(x=>x.Comments).FirstOrDefaultAsync(x => x.Id == ticketId));
                return ticket;
            }
            catch (Exception e)
            {
